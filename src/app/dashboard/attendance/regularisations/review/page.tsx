...
export default function RegularisationReviewsPage() {
  ...
  const fetchRegularisations = async () => {
    try {
      setLoading(true);
      const res = await api.get('/attendance/regularise/pending');
      setData(res.data?.data || []);
    } catch (err) {
      console.error('Failed to fetch regularisations', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReview = async (id: string, status: string) => {
    try {
      await api.put(`/attendance/regularise/${id}/review`, { status, notes: reviewNotes });
      setShowDrawer(false);
      setReviewNotes('');
      fetchRegularisations();
      alert(`Regularisation request ${status.toLowerCase()}`);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900">Regularisation Approvals</h1>
          <p className="text-zinc-500 mt-1">Review and manage attendance correction requests.</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={fetchRegularisations} className="flex items-center gap-2 px-4 py-2 bg-white border border-zinc-200 text-zinc-700 rounded-xl hover:bg-zinc-50 font-medium transition-colors text-sm">
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-zinc-50 border-b border-zinc-200 text-zinc-500">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Att. Date</th>
                <th className="px-6 py-4 font-medium">Req In</th>
                <th className="px-6 py-4 font-medium">Req Out</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center text-zinc-500">
                    No pending regularisation requests found.
                  </td>
                </tr>
              ) : data.map((req) => (
                  <tr key={req.id} className="hover:bg-zinc-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium text-zinc-900">{new Date(req.attendance_date).toLocaleDateString()}</td>
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-2xl z-50 transform transition-transform animate-in slide-in-from-right flex flex-col">
            <div className="p-6 border-b border-zinc-100 flex items-center justify-between shrink-0">
              <div>
                <h2 className="text-lg font-bold text-zinc-900">Review Regularisation</h2>
                <p className="text-sm text-zinc-500">{selectedReq.full_name} • {new Date(selectedReq.attendance_date).toLocaleDateString()}</p>
              </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              <div className="flex items-center justify-between p-4 bg-zinc-50 rounded-2xl">
                ...
                <div className="text-right">
                  ...
                  <p className="mt-1 font-medium text-zinc-900">{new Date(selectedReq.created_at).toLocaleDateString()}</p>
                </div>
              </div>

              <div>
                <h3 className="text-sm font-bold text-zinc-900 mb-3">Requested Changes</h3>
                <div className="space-y-3 bg-white border border-zinc-100 rounded-2xl p-4">
                  <div className="flex justify-between items-center text-sm pb-3 border-b border-zinc-100">
                    <span className="text-zinc-500">Current Status</span>
                    {getAttendanceBadge('UNKNOWN')}
                  </div>
                  <div className="flex justify-between items-center text-sm pt-1">
                    <span className="text-zinc-500">Requested In</span>
                    <span className="font-medium text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">-</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-zinc-500">Requested Out</span>
                    <span className="font-medium text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">-</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h3 className="text-sm font-bold text-zinc-900 mb-3">Reason Provided</h3>
              ...
              {selectedReq.status === 'PENDING' && (
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 mb-3">Manager Notes (Optional)</h3>
                  <textarea 
                    rows={3} 
                    value={reviewNotes}
